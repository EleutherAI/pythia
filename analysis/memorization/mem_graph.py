...
def rate_of_memorization(models, checkpoints, _idx):
    ...
    for model in models:
        ...
        for idx, checkpoint in enumerate(checkpoints):
            ...
            if idx > _idx:
                upper_max_sequence_index = int(n_steps[idx]) * 1024
                lower_max_sequence_index = int(n_steps[idx-1] if idx-1 >= 0 else 0) * 1024

                evals = memorization_results[f'{model}-{checkpoint}']
                # evals = evals[evals['index'] < max_sequence_index]
                evals = evals[(evals['index'] < upper_max_sequence_index) & (evals['index'] >= lower_max_sequence_index)]
                prediction = evals['accuracy'] == 1
                prediction = [prediction.mean()*100]
            else:
                prediction = [None]
...
ax.set_ylabel("Memorized Sequences (%)") #, fontsize=14) #, labelpad=20)
size = "23m"
plt.savefig('{}_graph_num_memorized.svg'.format(size), dpi=300)
plt.savefig('{}_graph_num_memorized.png'.format(size), dpi=300)
plt.clf()

checkpoint_names = ['23m', '44m', '65m', '85m', '105m', '126m', '146m']

for idx, size in enumerate(checkpoint_names):
    df = rate_of_memorization(models, checkpoint_names, idx)
    ax = sns.lineplot(data=df, x="checkpoint", y="num_memorization", hue="model")
    ax.set_title("Number of Memorized Sequences Seen from Earliest Checkpoint")
    ax.set_xlabel("Checkpoint") #, fontsize=14) #, labelpad=20)
    ax.set_ylabel("Memorized Sequences (%)") #, fontsize=14) #, labelpad=20)
